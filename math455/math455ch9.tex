...
\begin{definition}[Cauchy] A sequence $\{ x_n \}$ in a metric space $(X, \rho)$ is said to be a \textbf{Cauchy sequence} if $\forall \varepsilon > 0$, there exists a $N \in \mathbb{N}$ such that if $m, n \geq N$, then $\rho(x_n, x) < \varepsilon$. 
\end{definition}
...
\begin{definition}[Sequentially Compact]
	A metric space $X$ is \textbf{sequentially compact} if every sequence $X$ has a subsequence that converges to a point in $X$. 
\end{definition}
